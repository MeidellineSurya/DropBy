...
class Group(Base):
    ...
    __table_args__ = (
        CheckConstraint("min_required > 0", name="ck_group_min_positive"),
        CheckConstraint("max_allowed >= min_required", name="ck_group_size_range"),
        Index("ix_groups_drop_status", "drop_id", "status"),
    )
    ...
    status: Mapped[GroupStatus] = mapped_column(
        Enum(GroupStatus), default=GroupStatus.forming
    )
...
class GroupMember(Base):
    ...
    status: Mapped[GroupMemberStatus] = mapped_column(
        Enum(GroupMemberStatus), default=GroupMemberStatus.invited
    )
